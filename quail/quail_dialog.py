...
class QuailDialog(QtWidgets.QDialog, FORM_CLASS):
    """
    /***************************************************************************
     QuailDialog
                                     A QGIS plugin
     This plugin is designed to get biodiveristy data into QGIS
    ...
    def __init__(self, parent=None):
        ...
        execute upon click.
        """
        super(QuailDialog, self).__init__(parent)
        ...
        self.checkTaxonBackbonePushButton.clicked.connect(self.check_backbone)
    ...
    def _populate_data_profiles(self):
        ...
        # set data profiles self variable to storage variable
    ...
    def _populate_reasons(self):
        ...
        # set reasons self variable to storage variable
     ...
     *                                                                         *
     *   close_spatial_dialogBox:                                              *
     *       close a spatial dialogBox object; also assign a specific          *
     *       dictionary if the user has closed the box without assigning       *
     *       columns                                                           *
     *   close_species_list_dialogBox:                                         *
     *       close a species list dialogBox object; also assign a specific     *
     *       dictionary if the user has closed the box without assigning       *
     *       columns                                                           *
     ...
     *   copy_to_clipboard:                                                    *
     *       copy a value (specifically a DOI) to the user's computer          *
     *       clipboard                                                         *
     *   get_spatial_selections_dialogBox:                                     *
     *       gets user input of what columns will be used for filtering in the *
     *       spatial tab                                                       *
     *   get_data_layers_for_species_list:                                     *
     *       gets user input of what data layers to use for species list       *
     *   get_taxon_column_names:                                               *
     *       gets the selected columns from each taxonomic combo box - these   *
     *       will be used to query galah-python                                *
     *   show_doi_messagebox:                                                  *
     *       gives the user a DOI for their query, and allows them to copy it  *
     *       to the clipboard                                                  *
     ...
     *   show_layers_for_species_list:                                         *
     *       gives the user a list of layers to compile data for               *
    ...
    def get_data_layers_for_species_list(self, dialogBox=None):
        ...
        # get checkboxes from dialog box and initialise layers
    ...
    def get_taxon_column_names(self):
        ...
        labels = [x for x in self.Taxonomy.children() if type(x) is QLabel and x.objectName() != "speciesNamesLabel"]
    ...
    def show_doi_messagebox(self, doi=None, title=None):
        ...
        label.move(40, 40)  # +2 to offset?
        label.resize(250, 50)

        # button
        msg.setButton = QPushButton("Copy DOI", msg)
        msg.setButton.move(310, 50)
    ...
    def show_spatial_dialogbox(self, title=None, selection_dict=None):
        ...
        msg.setButton = QPushButton("Select", msg)
    ...
    def parse_basis_of_record(self):
        ...
        # create appropriate filters for this and return as string
        if len(bor) > 0:
            return f"{bor_dict[atlas]}={",".join(bor)}"
        else:
            return None
        ...
        # otherwise, we have a valid datetime window
            ...
            if endDate == datetime.now().date():
                ...
                if atlas in ["Global", "GBIF"]:
                    ...
                    startDateString = f"{eventdate_dict[atlas]}>={datetime(d1, m1, y1).isoformat()}Z"
                    endDateString = f"{eventdate_dict[atlas]}=<{datetime(d2, m2, y2).isoformat()}Z"
    ...
    def parse_present_absent(self):
        ...
        # create appropriate filters for this and return as string
        if len(status) > 0:
            return f"{occstatus_dict[atlas]}={",".join(status)}"
        else:
            return None
    ...
    def upload_species_list(self):
        ...
        if dialog.exec():
            ...
            if file_name[0].endswith(".csv"):
                ...
                self.speciesFileLineEdit.setText(_translate("QuailDialogBase", file_name[0]))
    ...
    def show_taxonomic_columns(self):
        ...
        if hasattr(self, "speciesFileLineEdit"):
            ...
            if self.speciesFileLineEdit.text() not in [None, ""]:
                ...
                if len(extant_lb) > 0:
                    for lb in extant_lb:
                        if lb.objectName() != "speciesNamesLabel":
                            lb.deleteLater()
                ...
                for i, key in enumerate(taxon_selections):
                    ...
                    if len(dwc_terms) > 0 and label.objectName() in dwc_terms:
                        comboBox.setCurrentText(label.objectName())
                    comboBox.show()
    ...
    def check_backbone(self):
        ...
        if "matchType" in taxa_table.columns:
            match_statistics = {
    ...
    def get_species_list_counts_statuses(self):
        ...
        # loop over all layers and put data in dictionary
        for l in layers_to_compile:
        ...
        # get counts of each scientific names
        # try this
        counts = (
            df.groupby(by=["scientificName", "taxonConceptID", "stateConservation", "countryConservation"])
            .size()
        ...
        self.download_species_list(species_list=counts)
